package io.github.danjos.mybankapp.blocker.client;
...
public class ExchangeClient {
    ...
    public ConversionResponseDTO convert(ConversionRequestDTO request) {
        try {
            ...
            // Convert response back to BlockerService DTO
    ...
    public ConversionResponseDTO convertFallback(ConversionRequestDTO request, Exception ex) {
        log.warn("Fallback: Unable to convert currency, using approximate hardcoded rates");
        
        BigDecimal convertedAmount;
        BigDecimal exchangeRate;
        
        // Fallback to hardcoded rates if exchange service is unavailable
        if (request.getFromCurrency() == Currency.RUB) {
            convertedAmount = request.getAmount();
            exchangeRate = BigDecimal.ONE;
        } else if (request.getFromCurrency() == Currency.USD) {
            convertedAmount = request.getAmount().multiply(BigDecimal.valueOf(100)); // 1 USD = 100 RUB
            exchangeRate = BigDecimal.valueOf(100);
        } else if (request.getFromCurrency() == Currency.CNY) {
            convertedAmount = request.getAmount().multiply(BigDecimal.valueOf(14)); // 1 CNY = 14 RUB
            exchangeRate = BigDecimal.valueOf(14);
        } else {
            convertedAmount = request.getAmount();
            exchangeRate = BigDecimal.ONE;
        }
